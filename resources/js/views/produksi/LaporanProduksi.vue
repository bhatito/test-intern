...
// Export laporan ke Excel - FIXED VERSION
const exportLaporan = async (laporanId) => {
    try {
        

        // Gunakan axios dengan responseType blob
        const response = await axios.get(`/api/produksi/laporan/${laporanId}/export-excel`, {
            params: { token },
            responseType: 'blob',
            headers: {
        ...
        // Dapatkan filename dari header atau gunakan default
        const contentDisposition = response.headers['content-disposition'];
        let filename = `Laporan_Produksi_${laporanId}.xlsx`;
        ...
        // Coba baca error message dari blob jika ada
        if (error.response && error.response.data instanceof Blob) {
            try {
                const errorText = await error.response.data.text();
                const errorData = JSON.parse(errorText);
                errorMessage.value = errorData.message || 'Gagal export laporan Excel';
            } catch {
                errorMessage.value = 'Format file tidak valid atau terjadi kesalahan server';
            }
        } else {
            errorMessage.value = error.response?.data?.message || 'Gagal export laporan Excel';
        }
    }
}
...
const viewLaporanDetail = async (laporanItem) => {
    try {
        ...
        const response = await axios.get(`/api/produksi/laporan/${laporanItem.id}/export-excel?preview=true`)
            ...
            <div class="bg-gray-50 rounded-lg p-4">
              ...
              <div class="space-y-2 text-sm">
                ...
                <div class="flex justify-between">
                  ...
                  <span class="font-medium">{{ laporanDetail.laporan?.pembuat?.name }}</span>
                </div>
                <div class="flex justify-between">
